...
import { 
  ...
  Users, ClipboardList, LogOut, Trophy, Shield, MessageSquare, GitGraph, Copy, Menu, X, Filter, Activity
} from 'lucide-react';
...
const Layout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  return (
    <div className="min-h-screen flex">
      ...
      {/* Main Content */}
      <main className="flex-1 p-4 lg:p-8 lg:pt-8 pt-16 overflow-auto scrollbar-glass">
        {children}
      </main>
    </div>
  );
...
const MobileNav: React.FC<{
  ...
  return (
    <>
      {/* Fixed top bar - always visible on mobile */}
      <div className="lg:hidden fixed top-0 left-0 right-0 z-50 bg-background/95 backdrop-blur-md border-b border-border/30 px-4 py-3">
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-bold font-display gradient-text">SponsorHub</h2>
          <button onClick={() => setOpen(!open)} className="p-2 rounded-lg hover:bg-muted/30 transition-colors">
      ...
      {open && (
        <div className="lg:hidden fixed inset-0 z-40 bg-background/98 backdrop-blur-md pt-16 animate-fade-in">
          <div className="h-full overflow-y-auto p-4 space-y-1">
            {profile && (
              <div className="px-3 py-3 mb-3 border-b border-border/30">
            ...
            {userLinks.map(link => (
              <NavLink
                ...
                className={({ isActive }) =>
                  `flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium ${isActive ? 'text-primary bg-primary/10' : 'text-muted-foreground hover:text-foreground hover:bg-muted/20'}`
                }
              >
            ...
            {adminLinks.length > 0 && (
              <>
                <div className="pt-4 pb-2 px-4 text-xs font-semibold text-muted-foreground uppercase tracking-wider flex items-center gap-2">
                  <Shield className="w-3 h-3" /> Admin
                </div>
                {adminLinks.map(link => (
                  <NavLink
            ...
            <div className="pt-4 mt-4 border-t border-border/30">
              <button onClick={onSignOut} className="flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium text-destructive hover:bg-destructive/10 w-full">
